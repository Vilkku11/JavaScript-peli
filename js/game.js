...
export default class Game {
    ...
    checkCollison(rect1, rect2) {   //palauttaa toden jos kaksi neliötä ovat toistensa sisällä
    ...
    checkPlayerBottomCollision(rectBottom, rectFull) {    //palauttaa toden, jos minkään pelaajan alareuna on recFullin sisällä
        let hit = false;
        rectBottom.x.forEach(x => {
            if(x < (rectFull.x + rectFull.width) &&
    ...
    startUp() {
        ...
        let startingY = Math.floor(Math.random() * 51) + this.height - this.player.height - 50;
        ...
        this.maxJumpHeight = (this.player.minSpeedY * this.player.minSpeedY) / (2 * this.player.gravity);
        ...
        for (let i = startingY; i > 0; i -= 60) {
            let x = Math.floor(Math.random() * (this.width- this.platforms[0].width));
    ...
    updatePlayer() {
        ...
        this.platforms.forEach(platform => {
            if(this.checkPlayerBottomCollision(this.player, platform) && (this.player.speedY > 0)) {
                this.player.speedY = this.player.minSpeedY;
            }
        });
        ...
        if((this.player.y + this.player.speedY) <= (this.height * 0.5 - (this.player.height * 0.5))) {
            this.touchedMidwayPoint = true;
            
            this.player.y = (this.height - this.player.height) * 0.5;
            rollDown = (this.height - this.player.height) * 0.5 - this.player.y - this.player.speedY;
    ...
    createPlatforms() {
        ...
        if(this.points < 1500 && this.platforms[this.platforms.length - 1].y > 27) {
            let x = Math.floor(Math.random() * (this.width- this.platforms[0].width));
            let y = Math.floor(Math.random() * -33);
            this.platforms.push(new Platform(this, x, y));


        } else if (this.points < 4000 && this.platforms[this.platforms.length - 1].y > 40) {
            let x = Math.floor(Math.random() * (this.width - this.platforms[0].width));
            let y = Math.floor(Math.random() * -50);
            this.platforms.push(new Platform(this, x, y));


        } else if (this.points < 6000 && this.platforms[this.platforms.length - 1].y > 40) {
            let x = Math.floor(Math.random() * (this.width - this.platforms[0].width));
            let y = Math.floor(Math.random() * -90);

            if (Math.floor(Math.random() * 7) === 6 && this.platsAfterLastMovingPlat > 3) {
        ...
        } else if (this.points < 10000 && this.platforms[this.platforms.length - 1].y > 90) {
            let x = Math.floor(Math.random() * (this.width - this.platforms[0].width));
            ...
            } else if (Math.floor(Math.random() * 7) === 6 && this.platsAfterLastMovingPlat > 4) {
        ...
        } else if (this.points < 15000 && this.platforms[this.platforms.length - 1].y > 90) {
            let x = Math.floor(Math.random() * (this.width - this.platforms[0].width));
            ...
            if (this.movingPlatCount > 0) {
                ...
                this.platforms.push(new MovingPlatform(this, x, y));
                this.movingPlatCount--;
            } else if (Math.floor(Math.random() * 5) === 6 && this.platsAfterLastMovingPlat > 3) {
                //luodaan uusi setti liikkuvia platformeja kunhan viime setistä välissä oli 3 normaalia platformia
                this.platforms.push(new MovingPlatform(this, x, y));
        ...
        } else if(this.platforms[this.platforms.length - 1].y > this.maxJumpHeight - 20) {
            ...
            } while (!(x < this.platforms[this.platforms.length - 1].x - 100 ||
            ...
            } else if (Math.floor(Math.random() * 3) === 2 && this.platsAfterLastMovingPlat > 3) {
                //luodaan uusi setti liikkuvia platformeja kunhan viime setistä välissä oli 3 normaalia platformia
                this.platforms.push(new MovingPlatform(this, x, y));
    ...
    draw(context) {
        ...
        this.platforms.forEach(platform => {
            let jumpheight = (this.player.minSpeedY * this.player.minSpeedY) / (2 * this.player.gravity);
            platform.draw(context, jumpheight);
        });
